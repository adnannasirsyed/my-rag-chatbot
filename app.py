...
# Streamlit UI
st.set_page_config(page_title="AI Teaching Assistant Demo", page_icon="🎓", layout="wide")
st.title("🎓 AI Teaching Assistant for Data Science & AI/ML")
...
if st.session_state.index is not None and st.session_state.embedder is not None:
        ...
        with st.expander(f"Q{i}: {item['query']} (Role: {role}, Tone: {tone})", expanded=is_last_item):
            ...
            with col1:
                ...
                if 'prompt_messages' in response and response['prompt_messages']:
                    ...
                    st.markdown(f"*Limited details: {'Blocked query—review input guardrails.' if 'blocked' in response.get('answer', '').lower() else 'Generation succeeded but prompts not logged. Check API call.'}*")
    ...
    if asked_accessibility:
        st.info("🦯 **Accessibility Tip:** Responses are structured with headings and lists for screen readers. For more guidance, reply with 'screen reader help'.")
    ...
    st.info("👆 Upload course files to start chatting. Example files: syllabus.pdf, project_FAQ.docx, notebook.ipynb.")
